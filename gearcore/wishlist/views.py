...
class WishlistRemoveView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not slug:
            return JsonResponse({'debug_message': "потрібен slug продукту", 'html': None, }, status=400)
        ...
        try:
            ...
            return JsonResponse({'debug_message': "не знайдено списку для користувача ", 'html': None, }, status=400)

        try:
            motorcycle = Motorcycle.objects.get(slug=slug)
        except Motorcycle.DoesNotExist:
            return JsonResponse({'debug_message': f"не знайдено товару за slug '{slug}'", 'html': None, }, status=400)

        try:
            wishlist_item = WishlistItem.objects.get(wishlist=wishlist, product=motorcycle)
